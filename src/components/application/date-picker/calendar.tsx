...
export const CalendarContextProvider = ({ children }: PropsWithChildren) => {
    ...
    // React Aria's CalendarContext onChange is typed to also accept range
    // arrays. This provider is single-date only, so narrow before storing.
    const handleChange = (next: DateValue | readonly DateValue[]) => onChange(Array.isArray(next) ? (next[0] ?? null) : (next as DateValue));
